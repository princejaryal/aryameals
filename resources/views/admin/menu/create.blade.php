...
@section('title', 'Arya Meals - Add Menu Item')
...
@section('content')
    <div class="p-6 space-y-8">
        ...
        <section class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-2xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">Add Menu Item</h1>
                <p class="text-sm text-purple-500 mt-1">Add new menu item to restaurant</p>
            </div>
        </section>
        ...
        <section class="bg-white rounded-2xl shadow-lg border border-purple-200">
            ...
            <form id="addMenuItemForm" enctype="multipart/form-data" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        @if(request('restaurant_id'))
                            <div class="w-full px-4 py-3 rounded-lg border border-purple-200 bg-purple-50 text-slate-900">
                                @php
                                    $selectedRestaurant = $restaurants->firstWhere('id', request('restaurant_id'));
                                @endphp
                                {{ $selectedRestaurant ? $selectedRestaurant->name . ' - ' . $selectedRestaurant->city : 'Restaurant not found' }}
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex gap-6">
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" name="price_type" value="dual" checked
                    ...
                    <div id="halfFullPricing" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-purple-700 mb-2">Half Plate Price</label>
                            <input type="number" name="half_plate_price" step="0.01" min="0"
                                   class="w-full px-4 py-3 rounded-lg border border-purple-200 bg-white text-slate-900 focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                                   placeholder="0.00">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="full_plate_price" step="0.01" min="0"
                                   class="w-full px-4 py-3 rounded-lg border border-purple-200 bg-white text-slate-900 focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                                   placeholder="0.00">
                        </div>
                    </div>
                    
                    <!-- Single Price (for beverages) -->
                    <div id="singlePricing" class="hidden">
                        <div>
                            <label class="block text-sm font-medium text-purple-700 mb-2">Price</label>
                            <input type="number" name="single_price" step="0.01" min="0"
                                   class="w-full px-4 py-3 rounded-lg border border-purple-200 bg-white text-slate-900 focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                                   placeholder="0.00">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="3"
                                  class="w-full px-4 py-3 rounded-lg border border-purple-200 bg-white text-slate-900 focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                                  placeholder="Describe the menu item, ingredients, etc."></textarea>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <div class="space-y-4">
                            <label class="flex items-center cursor-pointer">
                                ...
                                <span class="ml-3 text-sm text-purple-700">Make item available immediately</span>
                            </label>
    ...
    <script>
        ...
        document.getElementById('addMenuItemForm').addEventListener('submit', function(e) {
            ...
            // Only require restaurant_id and name
            let requiredFields = ['restaurant_id', 'name'];
        ...
        function submitForm(form) {
            ...
            // Pricing is handled in the controller based on price_type selection
            ...
            submitButton.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i> Adding Menu Item...';
        ...
        function highlightFieldError(fieldName) {
            const field = document.querySelector(`[name="${fieldName}"]`);
            if (field) {
                // Skip highlighting optional fields if they're empty
                if ((fieldName === 'half_plate_price' || fieldName === 'full_plate_price' || fieldName === 'single_price') && !field.value) {
                    return;
                }
