...
@section('title', 'Edit Platform Fee - AryaMeals Admin')
...
<div class="p-6 space-y-8">
    ...
    <section class="flex items-center justify-between">
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-2xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">Edit Platform Fee</h1>
                <p class="text-sm text-purple-500 mt-1">Update platform fee details</p>
            </div>
        </div>
        <div class="flex items-center space-x-4">
    ...
    <section class="bg-white rounded-xl shadow-sm border border-purple-100">
        <div class="p-6">
            <form action="{{ route('admin.platform-fees.update', $platformFee) }}" method="POST" class="space-y-6" id="platformFeeForm" novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="fee_type" data-required="true" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500" {{ $platformFee->fee_type ? 'disabled' : '' }}>
                            <option value="">Select Fee Type</option>
                            <option value="delivery" {{ $platformFee->fee_type === 'delivery' ? 'selected' : '' }}>Delivery Fee</option>
                            <option value="service" {{ $platformFee->fee_type === 'service' ? 'selected' : '' }}>Service Fee</option>
                            <option value="tax" {{ $platformFee->fee_type === 'tax' ? 'selected' : '' }}>Tax</option>
                            <option value="convenience" {{ $platformFee->fee_type === 'convenience' ? 'selected' : '' }}>Convenience Fee</option>
                            <option value="packaging" {{ $platformFee->fee_type === 'packaging' ? 'selected' : '' }}>Packaging Fee</option>
                        </select>
                        @error('fee_type')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        @if($platformFee->fee_type)
                            <p class="mt-1 text-xs text-gray-500">Fee type cannot be changed once created</p>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="fee_name" data-required="true" maxlength="255" value="{{ old('fee_name', $platformFee->fee_name) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="fee_type_calculation" data-required="true" id="calculationType" 
                            ...
                            <option value="fixed" {{ $platformFee->fee_type_calculation === 'fixed' ? 'selected' : '' }}>Fixed Amount</option>
                            <option value="percentage" {{ $platformFee->fee_type_calculation === 'percentage' ? 'selected' : '' }}>Percentage</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" min="0" value="{{ old('sort_order', $platformFee->sort_order) }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Fixed Amount -->
                    <div id="fixedAmountField" {{ $platformFee->fee_type_calculation !== 'fixed' ? 'class="hidden"' : '' }}>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-rupee-sign mr-2"></i>Fixed Amount (₹)
                        </label>
                        <input type="number" name="fee_amount" data-required="true" min="0" step="0.01" value="{{ old('fee_amount', $platformFee->fee_amount) }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                               placeholder="0.00">
                        @error('fee_amount')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <!-- Percentage -->
                    <div id="percentageField" {{ $platformFee->fee_type_calculation !== 'percentage' ? 'class="hidden"' : '' }}>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-percent mr-2"></i>Percentage (%)
                        </label>
                        <input type="number" name="fee_percentage" data-required="true" min="0" max="100" step="0.01" value="{{ old('fee_percentage', $platformFee->fee_percentage) }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                               placeholder="0.00">
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" maxlength="1000" 
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                              placeholder="Describe what this fee is for and how it's calculated...">{{ old('description', $platformFee->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                </div>
                ...
                <div>
                    <label class="flex items-center space-x-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $platformFee->is_active) ? 'checked' : '' }}
    ...
    <!-- Fee Details Card -->
    <section class="bg-white rounded-xl shadow-sm border border-purple-100 mt-6">
        <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-800 mb-4">
                <i class="fas fa-info-circle mr-2"></i>Current Fee Details
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <p class="text-sm text-gray-600 mb-1">Fee Type</p>
                    <p class="font-medium text-gray-800">{{ ucfirst($platformFee->fee_type) }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-600 mb-1">Current Status</p>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $platformFee->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        {{ $platformFee->is_active ? 'Active' : 'Inactive' }}
                    </span>
                </div>
                <div>
                    <p class="text-sm text-gray-600 mb-1">Current Calculation</p>
                    <p class="font-medium text-gray-800">
                        @if($platformFee->fee_type_calculation === 'percentage')
                            {{ $platformFee->fee_percentage }}%
                        @else
                            ₹{{ number_format($platformFee->fee_amount, 2) }}
                        @endif
                    </p>
                </div>
                <div>
                    <p class="text-sm text-gray-600 mb-1">Created</p>
                    <p class="font-medium text-gray-800">{{ $platformFee->created_at->format('M d, Y') }}</p>
                </div>
            </div>
        </div>
    </section>
